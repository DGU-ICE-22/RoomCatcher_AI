...
# MySQL에서는 INTEGER PRIMARY KEY AUTOINCREMENT 대신 INT AUTO_INCREMENT PRIMARY KEY 사용
    ...
    try:
        ...
        try:
            options = {
            "시스템에어컨여부": optnInfo.get("시스템에어컨여부", 0) if optnInfo else 0,
            "벽걸이에어컨여부": optnInfo.get("벽걸이에어컨여부", 0) if optnInfo else 0,
            "입식에어컨여부": optnInfo.get("입식에어컨여부", 0) if optnInfo else 0,
            "침대여부": (optnInfo.get("침대여부", 0)) if optnInfo else 0,
            "책상여부": (optnInfo.get("책상여부", 0)) if optnInfo else 0,
            "옷장여부": (optnInfo.get("옷장여부", 0)) if optnInfo else 0,
            "붙박이장여부": (optnInfo.get("붙박이장여부", 0)) if optnInfo else 0,
            "식탁여부": (optnInfo.get("식탁여부", 0)) if optnInfo else 0,
            "소파여부": (optnInfo.get("소파여부", 0)) if optnInfo else 0,
            "신발장여부": (optnInfo.get("신발장여부", 0)) if optnInfo else 0,
            "냉장고여부": (optnInfo.get("냉장고여부", 0)) if optnInfo else 0,
            "세탁기여부": (optnInfo.get("세탁기여부", 0)) if optnInfo else 0,
            "건조기여부": (optnInfo.get("건조기여부", 0)) if optnInfo else 0,
            "샤워부스여부": (optnInfo.get("샤워부스여부", 0)) if optnInfo else 0,
            "욕조여부": (optnInfo.get("욕조여부", 0)) if optnInfo else 0,
            "비데여부": (optnInfo.get("비데여부", 0)) if optnInfo else 0,
            "싱크대여부": (optnInfo.get("싱크대여부", 0)) if optnInfo else 0,
            "식기세척기여부": (optnInfo.get("식기세척기여부", 0)) if optnInfo else 0,
            "가스레인지여부": (optnInfo.get("가스레인지여부", 0)) if optnInfo else 0,
            "인덕션레인지여부": (optnInfo.get("인덕션레인지여부", 0)) if optnInfo else 0,
            "베란다여부": (optnInfo.get("베란다여부", 0)) if optnInfo else 0,
            "자체경비원여부": (optnInfo.get("자체경비원여부", 0)) if optnInfo else 0,
            "비디오전화여부": (optnInfo.get("비디오전화여부", 0)) if optnInfo else 0,
            "인터폰여부": (optnInfo.get("인터폰여부", 0)) if optnInfo else 0,
            "cctv여부": (optnInfo.get("cctv여부", 0)) if optnInfo else 0,
            "방범창여부": (optnInfo.get("방범창여부", 0)) if optnInfo else 0,
            "현관보안여부": (optnInfo.get("현관보안여부", 0)) if optnInfo else 0,
            "무인택배박스여부": (optnInfo.get("무인택배박스여부", 0)) if optnInfo else 0,
            "엘리베이터여부": (optnInfo.get("엘리베이터여부", 0)) if optnInfo else 0,
            "테라스여부": (optnInfo.get("테라스여부", 0)) if optnInfo else 0,
            "마당여부": (optnInfo.get("마당여부", 0)) if optnInfo else 0
        }
        except Exception as e:
            print(f"Failed to process options: {e}")
            traceback.print_exc()
        ...
        ''',(
                ...
                item.get('매물주차가능여부', ''),
                item.get('매물주차대수', 0), 
                item.get('세대당주차대수비율', 0), 
                item.get('난방방식명', ''),
                item.get('내진설계여부', ''), 
                item.get('방거실형태명', ''), 
                item.get('원룸구조명', ''),
                management_costs.get("관리비", ''), 
                management_costs.get("관리비_전기세여부", ''),
                management_costs.get("관리비_가스여부", ''), 
                management_costs.get("관리비_수도여부", ''),
                management_costs.get("관리비_인터넷여부", ''), 
                management_costs.get("관리비_tv여부", ''),
                options.get("시스템에어컨여부", 0), 
                options.get("벽걸이에어컨여부", 0),
                options.get("입식에어컨여부", 0), 
                options.get("침대여부", 0), 
                options.get("책상여부", 0),
                options.get("옷장여부", 0), 
                options.get("붙박이장여부", 0), 
                options.get("식탁여부", 0),
                options.get("소파여부", 0), 
                options.get("신발장여부", 0), 
                options.get("냉장고여부", 0),
                options.get("세탁기여부", 0), 
                options.get("건조기여부", 0), 
                options.get("샤워부스여부", 0),
                options.get("욕조여부", 0), 
                options.get("비데여부", 0), 
                options.get("싱크대여부", 0),
                options.get("식기세척기여부", 0), 
                options.get("가스레인지여부", 0),
                options.get("인덕션레인지여부", 0), 
                options.get("베란다여부", 0),
                options.get("자체경비원여부", 0), 
                options.get("비디오전화여부", 0),
                options.get("인터폰여부", 0), 
                options.get("cctv여부", 0), 
                options.get("방범창여부", 0),
                options.get("현관보안여부", 0), 
                options.get("무인택배박스여부", 0),
                options.get("엘리베이터여부", 0), 
                options.get("테라스여부", 0), 
                options.get("마당여부", 0),
                item.get('사용승인일', ''), 
                item.get('수정일시', '')
